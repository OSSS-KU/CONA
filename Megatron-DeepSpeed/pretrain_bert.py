...
def loss_func(loss_mask, sentence_order, lm_labels, output_tensor):
    args = get_args()
    lm_loss_, sop_logits, lm_logits = output_tensor
    ...
    # Optional BERT metric: masked-token top-1 accuracy.
    # Only computed if model returned lm_logits (enabled via env var BERT_RETURN_LM_LOGITS=1).
    mlm_acc = None
    if lm_logits is not None:
        # lm_logits is [s, b, vocab_parallel]. Labels/masks are [b, s].
        # For TP=1 this is exact; for TP>1, argmax over vocab-parallel shards would need extra collectives.
        if args.tensor_model_parallel_size == 1:
            # Check if lm_logits is not empty (avoid argmax on empty tensor)
            if lm_logits.numel() > 0:
                with torch.no_grad():
                    labels_sb = lm_labels.transpose(0, 1).contiguous()  # [s, b]
                    mask_sb = labels_sb.ne(-1)
                    denom = mask_sb.sum().clamp(min=1).float()
                    pred_sb = lm_logits.float().argmax(dim=-1)
                    correct = pred_sb.eq(labels_sb) & mask_sb
                    mlm_acc = correct.sum().float() / denom
        # else: leave as None (avoid wrong metric).
    ...
    if sop_logits is not None:
        ...
        if mlm_acc is None:
            averaged_losses = average_losses_across_data_parallel_group(
                [lm_loss])
            return loss, {'lm loss': averaged_losses[0]}
        averaged = average_losses_across_data_parallel_group([lm_loss, mlm_acc])
        return loss, {'lm loss': averaged[0],
                      'mlm acc': averaged[1]}
...
def forward_step(data_iterator, model):
    ...
    return output_tensor, partial(loss_func, loss_mask, sentence_order, lm_labels)
