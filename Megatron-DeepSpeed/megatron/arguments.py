...
def validate_args(args, defaults={}):
    ...
    # --checkpoint-activations is retained: DeepSpeed drives its own activation
    # checkpointing through it rather than through --recompute-granularity.
    ...
    # DeepSpeed does not support async_tensor_model_parallel_allreduce.
    if args.deepspeed:
        args.async_tensor_model_parallel_allreduce = False
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--cona-candidate-file', type=str, default=None,
                       help='CONA: file the strategy search publishes its next '
                            'candidate strategy to. Scored once per iteration '
                            'against the strategy this job is running under.')
    group.add_argument('--cona-switch-file', type=str, default=None,
                       help='CONA: file to record the switch in before '
                            'checkpointing and exiting, so the runner knows '
                            'which strategy the next stage should run under.')
    group.add_argument('--cona-fallback-file', type=str, default=None,
                       help='CONA: file to record a fallback in before '
                            'checkpointing and exiting. Passing it enables the '
                            'fallback for sustained GNS decreases; without it '
                            'the fallback never runs.')
    group.add_argument('--cona-fallback-ema-alpha', type=float, default=0.1,
                       help='CONA: smoothing factor of the exponential moving '
                            'average the fallback tracks GNS with.')
    group.add_argument('--cona-fallback-patience', type=int, default=100,
                       help='CONA: consecutive iterations the smoothed GNS must '
                            'decrease for before the fallback reverts.')
    group.add_argument('--cona-fallback-prev-gbs', type=int, default=None,
                       help='CONA: global batch size of the previously passed '
                            'strategy, logged with the batch size below which '
                            'it would become preferable again.')
    group.add_argument('--cona-fallback-prev-time-sec', type=float, default=None,
                       help='CONA: iteration time of the previously passed '
                            'strategy, in seconds.')
    return parser
...
def _add_training_args(parser):
    ...
    # Retained for DeepSpeed's activation checkpointing.
...
def _add_learning_rate_args(parser):
    ...
    group.add_argument('--bert-override-lr-only', action='store_true',
                       help='BERT-only: when resuming with '
                       '--use-checkpoint-opt_param-scheduler, override only '
                       'learning rate (lr/min-lr) from input arguments after '
                       'loading scheduler state from the checkpoint. This '
                       'enables staged runs where GBS changes and LR is '
                       'recomputed per stage without changing other scheduler '
                       'state. Has no effect unless explicitly set.')
