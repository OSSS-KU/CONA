...
def _exec_backward_only_pass(self, buffer_id):
    ...
    # DeepSpeed's Engine installs a backward hook which errors out if it thinks the
    # user called backward() directly without scaling the loss.
    #
    # In pipeline-parallel training, the last stage handles true loss scaling on the
    # scalar loss. Upstream stages receive already-scaled activation gradients, so
    # they must NOT call engine.scale() on non-scalar tensors.
    #
    # We only need to mark that a "manual backward" is expected so the hook does
    # not raise for intermediate stages.
