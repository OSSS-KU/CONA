...
def _patch_deepspeed_pipeline_backward():
    """Patch DeepSpeed PipelineEngine._exec_backward_pass to handle fp16 loss scaling.
    
    When using fp16 with loss scaling (loss_scale: 0), DeepSpeed requires that
    backward() is called through its scaling mechanism. However, in intermediate
    pipeline stages, torch.autograd.backward() is called directly, which bypasses
    DeepSpeed's loss scaling check.

    In pipeline-parallel training, the *last* stage performs the true loss scaling
    (via DeepSpeed's ZeRO scaler / GradScaler) and the activation gradients sent
    upstream are already scaled. Therefore, intermediate stages must NOT attempt
    to call `engine.scale()` on non-scalar tensors. They only need to mark that a
    "manual backward" is expected so DeepSpeed's backward hook does not raise.
    """
    import deepspeed.runtime.pipe.engine as pipe_engine
    import deepspeed.runtime.engine as ds_engine

    original_exec_backward_pass = pipe_engine.PipelineEngine._exec_backward_pass

    def patched_exec_backward_pass(self, buffer_id):
        # DeepSpeed's Engine installs a backward hook which errors out if it
        # thinks the user called backward() directly without scaling the loss.
        # PipelineEngine calls torch.autograd.backward() internally for ALL stages.
        # Mark "manual backward expected" so the hook does not raise.
        #
        # This is safe because the loss scaling happens on the last stage's loss,
        # and upstream stages receive already-scaled activation gradients.
        self._manual_backward_expected = True
        return original_exec_backward_pass(self, buffer_id)

    pipe_engine.PipelineEngine._exec_backward_pass = patched_exec_backward_pass

    # Extra safety: some code paths end up calling the Engine backward post hook
    # even when PipelineEngine invoked torch.autograd.backward internally.
    # Ensure the flag is set for PipelineEngine instances so the hook does not raise.
    original_backward_post_hook = ds_engine.DeepSpeedEngine._backward_post_hook

    def patched_backward_post_hook(self):
        # Only relax the check for the DeepSpeed PipelineEngine path.
        if self.__class__.__name__ == "PipelineEngine" and \
           getattr(self.__class__, "__module__", "") == "deepspeed.runtime.pipe.engine":
            self._manual_backward_expected = True
        return original_backward_post_hook(self)

    ds_engine.DeepSpeedEngine._backward_post_hook = patched_backward_post_hook
...
def _compile_dependencies():
    ...
    # Load fused kernels
    # ==================
    # Fast-start / reliability: allow an explicit opt-out via env var.
    # Fused kernel JIT can hang or take a long time on some environments.
    if os.getenv("MEGATRON_DISABLE_FUSED_KERNELS", "0") in ("1", "true", "True", "YES", "yes"):
        if args.rank == 0:
            print("> MEGATRON_DISABLE_FUSED_KERNELS is set; skipping fused kernels load/compile.", flush=True)
        return
...
def _initialize_distributed():
    ...
    if args.enable_zbh1_pipeline:
        ...
        # Patch standard DeepSpeed pipeline engine for fp16 loss scaling
        # This is needed when using fp16 with loss_scale: 0 (dynamic loss scaling)
        _patch_deepspeed_pipeline_backward()
