...
def _load_npy_index_cache(path, *, max_wait_sec=600, poll_sec=0.25):
    """Load a cached .npy mapping; retry for shared filesystems (e.g. NFS).

    Rank 0 writes index-cache files; other nodes can see FileNotFoundError until
    the server propagates metadata. all_reduce does not wait for NFS visibility.
    """
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    if build_indices and is_rank_0():
        ...
        try:
            ...
            print_rank_0(' > elasped time to build and save shuffle-idx mapping'
                         ' (seconds): {:4f}'.format(time.time() - start_time))
            # Help NFS / network FS: sync directory so other nodes can see new files.
            try:
                _dfd = os.open(data_cache_dir, os.O_RDONLY)
                os.fsync(_dfd)
                os.close(_dfd)
            except OSError:
                pass
    ...
    # Do not call torch.distributed.barrier() here: with tensor parallel (TP>1),
    # NCCL barrier/broadcast can deadlock or time out across process groups while
    # ranks wait on NFS; all_reduce above already synchronized success, and
    # _load_npy_index_cache retries until files are visible.

    # Load mappings.
    start_time = time.time()
    print_rank_0(f" > loading doc-idx mapping from {idx_path['doc']}")
    doc_idx = _load_npy_index_cache(idx_path['doc'])

    print_rank_0(f" > loading sample-idx mapping from {idx_path['sample']}")
    sample_idx = _load_npy_index_cache(idx_path['sample'])

    print_rank_0(f" > loading shuffle-idx mapping from {idx_path['shuffle']}")
    shuffle_idx = _load_npy_index_cache(idx_path['shuffle'])
