...
_ADAPTDL_ENABLED = os.getenv("ADAPTDL_LOG_EFFICIENCY", "0") == "1"
try:
    _ADAPTDL_INIT_BATCH_SIZE = int(os.getenv("ADAPTDL_INIT_BATCH_SIZE", "0") or 0)
except Exception:
    _ADAPTDL_INIT_BATCH_SIZE = 0
_ADAPTDL_EFFICIENCY_BATCH_SIZES = [8, 16, 32, 64, 128, 256]
try:
    eff_bsz_str = os.getenv("ADAPTDL_EFFICIENCY_BATCH_SIZES", "")
    if eff_bsz_str:
        _ADAPTDL_EFFICIENCY_BATCH_SIZES = [int(x.strip()) for x in eff_bsz_str.split(",") if x.strip()]
except Exception:
    pass
_ADAPTDL_EFFICIENCY_CSV_FILE = os.getenv("ADAPTDL_EFFICIENCY_CSV_FILE", None)
_ADAPTDL_CSV_HEADER_WRITTEN = False
_ADAPTDL_GNS = None
_ADAPTDL_DP_SIZE = None
_ADAPTDL_DS_ENGINE = None
_ADAPTDL_OPT_SHIM = None


_CONA_GNS = None
_CONA_ITER_SECONDS = []
_CONA_LAST_ITER_END = None
_CONA_RULE = None
_CONA_GNS_SEQ = 0
_CONA_FALLBACK_RULE = None
_CONA_FALLBACK_MONITOR = None
_CONA_FALLBACK_SEEN_SEQ = -1
_CONA_MIN_SAMPLES = 8
_CONA_WINDOW = 50


def _cona_switch_rule():
    """CONA's switch rule, loaded from the runner's copy of ``switch_rule.py``.

    Loaded by path so that the search and the training loop apply the same rule.
    """
    global _CONA_RULE
    if _CONA_RULE is None:
        import importlib.util

        path = os.getenv("CONA_SWITCH_RULE")
        if not path or not os.path.exists(path):
            raise RuntimeError(
                "CONA_SWITCH_RULE must point at the runner's switch_rule.py"
            )
        spec = importlib.util.spec_from_file_location("cona_switch_rule", path)
...
def _cona_fallback_rule():
    """CONA's fallback rule, loaded from the runner's copy of ``fallback.py``."""
    global _CONA_FALLBACK_RULE
    if _CONA_FALLBACK_RULE is None:
        import importlib.util

        path = os.getenv("CONA_FALLBACK_RULE")
        if not path or not os.path.exists(path):
            raise RuntimeError(
                "CONA_FALLBACK_RULE must point at the runner's fallback.py"
            )
        spec = importlib.util.spec_from_file_location("cona_fallback_rule", path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        _CONA_FALLBACK_RULE = module
    return _CONA_FALLBACK_RULE


def _cona_fallback_monitor(args):
    global _CONA_FALLBACK_MONITOR
    if _CONA_FALLBACK_MONITOR is None:
        rule = _cona_fallback_rule()
        _CONA_FALLBACK_MONITOR = rule.GnsFallbackMonitor(
            ema_alpha=args.cona_fallback_ema_alpha,
            patience=args.cona_fallback_patience,
        )
        print_rank_0(
            "[CONA] fallback armed: alpha={} patience={}".format(
                args.cona_fallback_ema_alpha, args.cona_fallback_patience
            )
        )
    return _CONA_FALLBACK_MONITOR


def _cona_fallback_fires(args, iteration):
    global _CONA_FALLBACK_SEEN_SEQ
    if _CONA_GNS_SEQ == _CONA_FALLBACK_SEEN_SEQ:
        return False
    _CONA_FALLBACK_SEEN_SEQ = _CONA_GNS_SEQ

    monitor = _cona_fallback_monitor(args)
    if not monitor.observe(_CONA_GNS):
        return False

    threshold = None
    if (
        args.cona_fallback_prev_gbs is not None
        and args.cona_fallback_prev_time_sec is not None
        and len(_CONA_ITER_SECONDS) >= _CONA_MIN_SAMPLES
    ):
        ordered = sorted(_CONA_ITER_SECONDS)
        t_cur = ordered[len(ordered) // 2]
        rule = _cona_fallback_rule()
        threshold = rule.reversal_gns_threshold(
            args.cona_fallback_prev_gbs,
            args.cona_fallback_prev_time_sec,
            args.global_batch_size,
            t_cur,
        )

    print_rank_0(
        "[CONA] iter={} FALLBACK: smoothed gns={:.4g} decreased for {} "
        "consecutive iterations (gns={:.1f}, ranking reverses below {})".format(
            iteration, monitor.ema, monitor.consecutive_decreases,
            _CONA_GNS if _CONA_GNS is not None else float("nan"),
            "n/a" if threshold is None else "{:.1f}".format(threshold),
        )
    )
    return True


def _cona_candidate_wins(args, iteration):
    """Whether the published candidate beats the strategy this job is running."""
    if _CONA_GNS is None or len(_CONA_ITER_SECONDS) < _CONA_MIN_SAMPLES:
        return False
    rule = _cona_switch_rule()
    candidate = rule.read_candidate(args.cona_candidate_file)
    ...
    t_cur = ordered[len(ordered) // 2]
    try:
        ratio = rule.ratio_from_scalars(
            args.global_batch_size,
            t_cur,
            float(candidate["gbs"]),
            float(candidate["t_next_sec"]),
            _CONA_GNS,
        )
    except ValueError:
        return False
    ...
    print_rank_0(
        "[CONA] iter={} gns={:.1f} gbs_cur={} T_cur={:.1f}ms "
        "next=dp{}_tp{}_pp{}_mbs{}_gbs{} T_next={:.1f}ms R={:.4f} {}".format(
            iteration, _CONA_GNS, args.global_batch_size, t_cur * 1e3,
            candidate.get("dp"), candidate.get("tp"), candidate.get("pp"),
            candidate.get("mbs"), candidate.get("gbs"),
            float(candidate["t_next_sec"]) * 1e3, ratio,
            "SWITCH" if ratio > 1.0 else "keep",
        )
    )
    return ratio > 1.0


_CONA_ACTIONS = {1: "switch", 2: "fallback"}


def _cona_maybe_switch(iteration):
    """One strategy decision, taken identically on every rank.

    Returns ``"switch"`` to advance to the published candidate, ``"fallback"``
    to revert to the previously passed strategy, or ``""`` to carry on. Only
    rank 0 measures GNS, so it decides and broadcasts the answer, and the
    collective checkpoint that follows is then entered by every rank. The
    fallback is checked first: it steps back down the ladder, not further up.
    """
    args = get_args()
    if not (args.cona_candidate_file or args.cona_fallback_file):
        return ""

    global _CONA_LAST_ITER_END
    now = time.time()
    if _CONA_LAST_ITER_END is not None:
        _CONA_ITER_SECONDS.append(now - _CONA_LAST_ITER_END)
        del _CONA_ITER_SECONDS[:-_CONA_WINDOW]
    _CONA_LAST_ITER_END = now

    action = 0
    if is_rank_0():
        try:
            if args.cona_fallback_file and _cona_fallback_fires(args, iteration):
                action = 2
            elif args.cona_candidate_file and _cona_candidate_wins(args, iteration):
                action = 1
        except Exception as exc:
            print_rank_0(f"[CONA] strategy check failed: {exc}")
            action = 0
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        ...
        flag = torch.tensor([action], dtype=torch.int32, device=device)
        torch.distributed.broadcast(flag, 0)
        action = int(flag.item())
    return _CONA_ACTIONS.get(action, "")


def _cona_record_switch(iteration, action="switch"):
    """Tell the runner what this job stopped on, where, and at what GNS.

    The second line carries ``GNS_t`` so that the next stage can scale its
    learning rate against the noise the run is actually seeing.
    """
    args = get_args()
    path = (
        args.cona_fallback_file if action == "fallback" else args.cona_switch_file
    )
    if not (is_rank_0() and path):
        return
    try:
        with open(path, "w") as handle:
            handle.write(f"{iteration}\n")
            if _CONA_GNS is not None:
                handle.write(f"{_CONA_GNS!r}\n")
    except OSError as exc:
        print_rank_0(f"[CONA] could not record the {action}: {exc}")


def _cona_record_gns(grad_sqr, grad_var):
    """Record ``GNS_t = grad_var / grad_sqr``, in samples, for CONA's switch rule.

    Measured where the gradient statistics are available and read at the end of
    the iteration, where stopping to checkpoint is safe.
    """
    global _CONA_GNS, _CONA_GNS_SEQ
    if grad_sqr > 0 and grad_var >= 0:
        _CONA_GNS = max(grad_var / grad_sqr, 0.0)
        _CONA_GNS_SEQ += 1


class _AdaptdlLossScaleShim:
    """Expose `get_scale()` compatible with AdaptDL's GradientNoiseScale.

    DeepSpeed's loss scaler object lives on the engine and is only available
    after `deepspeed.initialize`. We initialize GNS early (to register hooks
    before DeepSpeed reducer hooks) and use this shim to read the current scale
    lazily once the engine is set.
    """
    def get_scale(self):
        eng = _ADAPTDL_DS_ENGINE
        try:
            ls = getattr(eng, "loss_scaler", None)
            if ls is None:
                return 1.0
            gs = getattr(ls, "get_scale", None)
            return float(gs()) if callable(gs) else 1.0
        except Exception:
            return 1.0


class _AdaptdlSyncShim:
    """Expose require_backward_grad_sync for AdaptDL GradientNoiseScale.

    GradientNoiseScale expects an object with a boolean attribute
    `require_backward_grad_sync` indicating whether this backward triggers
    gradient synchronization (i.e., gradient accumulation boundary).
    """
    def __init__(self, engine, gas: int = 1):
        self._engine = engine
        try:
    ...
    def require_backward_grad_sync(self):
        self._mb_counter += 1
        is_boundary = (self._mb_counter % self._gas) == 0
        if os.getenv("ADAPTDL_GNS_DEBUG", "0") == "1" and is_rank_0():
            print_rank_0(f"[ADAPTDL_GNS_DEBUG][rank0] shim: mb_counter={self._mb_counter} gas={self._gas} -> boundary={is_boundary}")
        return is_boundary


class _AdaptdlOptimizerShim:
    """Optimizer shim for AdaptDL GradientNoiseScale.

    DeepSpeed commonly uses master FP32 parameters in the optimizer which are
    NOT the same Parameter objects participating in autograd. AdaptDL's
    GradientNoiseScale registers backward hooks on `optimizer.param_groups`
    params, so we provide a shim optimizer whose param_groups reference the
    *model* parameters (which will actually receive grads).

    This shim is used ONLY for gradient statistics; it does not participate in
    stepping. We keep an independent `.state` dict for GNS.
    """
    def __init__(self, model_params, betas=(0.9, 0.999), eps=1e-8):
        self.param_groups = [{
...
def _maybe_init_adaptdl_gns(model, optimizer):
    global _ADAPTDL_GNS, _ADAPTDL_DP_SIZE, _ADAPTDL_OPT_SHIM
    if not _ADAPTDL_ENABLED or _ADAPTDL_GNS is not None:
        return

    try:
        from adaptdl.torch.gradient_noise_scale import GradientNoiseScale, AdamGradientNoiseScale
    except Exception as e:
        print_rank_0(f"[ADAPTDL_GNS] disabled: failed to import GradientNoiseScale ({e})")
        return

    engine_like = model[0] if isinstance(model, list) else model

    dp_group = None
    dp_size = None
    try:
        eng = _ADAPTDL_DS_ENGINE
        if eng is not None and hasattr(eng, "grid") and hasattr(eng.grid, "get_data_parallel_group"):
            dp_group = eng.grid.get_data_parallel_group()
            dp_size = getattr(eng.grid, "data_parallel_size", None)
    except Exception:
        dp_group = None
        dp_size = None
    if dp_group is None:
    ...
    try:
        ...
        if dp_size is None:
            dp_size = 1
        denom = int(args.micro_batch_size) * int(dp_size)
        gas = int(args.global_batch_size) // max(denom, 1)
        gas = max(gas, 1)
    except Exception:
        gas = 1

    adp_shim = _AdaptdlSyncShim(engine_like, gas=gas)

    mp_scaler = _AdaptdlLossScaleShim()

    try:
        try:
            model_params = list(engine_like.parameters())
        except Exception:
            model_params = []
        if not model_params:
            print_rank_0("[ADAPTDL_GNS] disabled: could not access model parameters for hooks")
            return
        betas = (0.9, 0.999)
        eps = 1e-8
        try:
            ...
            if hasattr(args, "adam_eps"):
                eps = float(args.adam_eps)
        except Exception:
            pass
        opt_shim = _AdaptdlOptimizerShim(model_params, betas=betas, eps=eps)
        _ADAPTDL_OPT_SHIM = opt_shim

        gns_cls = GradientNoiseScale
        try:
            base_opt = optimizer
            if hasattr(base_opt, "optimizer"):
                base_opt = base_opt.optimizer
            name = type(base_opt).__name__.lower() if base_opt is not None else ""
            if "adam" in name:
                gns_cls = AdamGradientNoiseScale
        except Exception:
            gns_cls = GradientNoiseScale

        try:
            gns_accum_scale = float(dp_size) * float(get_args().micro_batch_size)
        except Exception:
            gns_accum_scale = None

        _ADAPTDL_GNS = gns_cls(
            adp_shim,
            opt_shim,
            mp_scaler=mp_scaler,
            num_replicas=dp_size,
            accum_scale=gns_accum_scale,
            process_group=dp_group,
        )
        print_rank_0(
            f"[ADAPTDL_GNS] enabled: init_batch_size={_ADAPTDL_INIT_BATCH_SIZE}, "
            f"dp_size={dp_size}"
        )
    except Exception as e:
        print_rank_0(f"[ADAPTDL_GNS] disabled: failed to init GradientNoiseScale ({e})")
        _ADAPTDL_GNS = None
...
def megatron_quiesce_before_exit():
    """Exit without Py_Finalize to avoid ProcessGroupNCCL destructor crashes on multi-node PP.

    Logs often show failure during interpreter shutdown (not during NFS checkpoint read).
    W&B background upload + Py_Finalize races NCCL teardown; finish W&B then os._exit(0).
    """
    try:
...
def get_optimizer_param_scheduler(optimizer):
    ...
    # Sample-based training.
        ...
        # Training iterations are needed later on.
...
def setup_model_and_optimizer(model_provider_func,
    ...
    if args.deepspeed:
        ...
        _maybe_init_adaptdl_gns(model, optimizer)
        ...
        global _ADAPTDL_DS_ENGINE
        _ADAPTDL_DS_ENGINE = model[0]
        if _ADAPTDL_GNS is not None and _ADAPTDL_OPT_SHIM is not None:
            try:
                _bind_adaptdl_optimizer_state(model[0], _ADAPTDL_OPT_SHIM)
            except Exception:
                pass

    # Compression has its own checkpoint loading path (e.g, loading both teacher and student models).
    # So if compression is enabled, we skip the following checkpoint loading.
...
def _bind_adaptdl_optimizer_state(engine, opt_shim):
    """Populate opt_shim.state[param] from DeepSpeed master-param optimizer state.

    This enables AdaptDL's AdamGradientNoiseScale preconditioner to match the
    underlying optimizer without registering hooks on master params.
    """
    # Unwrap common DeepSpeed optimizers (e.g., ZeRO wrappers).
    opt = getattr(engine, "optimizer", None)
    if opt is None:
        return
    base_opt = getattr(opt, "optimizer", opt)

    fp16_groups = getattr(opt, "fp16_groups", None)
    fp32_groups = getattr(opt, "fp32_groups", None)
    if fp16_groups is None or fp32_groups is None:
        fp16_groups = getattr(base_opt, "fp16_groups", None)
        fp32_groups = getattr(base_opt, "fp32_groups", None)

    if fp16_groups is None or fp32_groups is None:
        # If the optimizer state is keyed by the same Parameter objects we hook,
        # nothing to bind.
        return
    ...
    # Map fp16 params to their corresponding fp32 master params by group order.
    for g16, g32 in zip(fp16_groups, fp32_groups):
        for p16, p32 in zip(g16, g32):
            if p32 in state:
                opt_shim.state[p16] = state[p32]


def train_step(forward_step_func, data_iterator,
               model, optimizer, opt_param_scheduler, config):
    """Single training step."""
    global _ADAPTDL_CSV_HEADER_WRITTEN
    args = get_args()
    timers = get_timers()
    ...
    if args.deepspeed and args.ds_pipeline_enabled:
        ...
        # already updated its statistics in autograd callbacks during backward
        # by the time was_step_applied() can be queried. The state is snapshotted
        # here and restored when the step turns out to have been skipped.
        _gns_state_backup = None
        _gns_prev_grads_backup = None
        if _ADAPTDL_GNS is not None:
            try:
                import copy
                _gns_state_backup = copy.deepcopy(_ADAPTDL_GNS._optimizer.state.get("gns", {}))
                _gns_prev_grads_backup = _ADAPTDL_GNS._prev_grads
            except Exception:
                _gns_state_backup = None
                _gns_prev_grads_backup = None
        ...
        if _ADAPTDL_GNS is not None and (not update_successful) and _gns_state_backup is not None:
            try:
                _ADAPTDL_GNS._optimizer.state["gns"] = _gns_state_backup
                _ADAPTDL_GNS._prev_grads = _gns_prev_grads_backup
            except Exception:
                pass

        if _ADAPTDL_GNS is not None and os.getenv("ADAPTDL_GNS_DEBUG", "0") == "1" and is_rank_0():
            try:
                ms = getattr(model[0], "micro_steps", None)
                gas = model[0].gradient_accumulation_steps() if hasattr(model[0], "gradient_accumulation_steps") else None
                mb_id = getattr(model[0], "microbatch_id", None)
                mb_total = getattr(model[0], "micro_batches", None)
                fn = getattr(model[0], "is_gradient_accumulation_boundary", None)
                ib = fn() if callable(fn) else None
                print_rank_0(f"[ADAPTDL_GNS_DEBUG][rank0] ds: micro_steps={ms} gas={gas} "
                             f"microbatch_id={mb_id} micro_batches={mb_total} "
                             f"is_grad_boundary={ib} was_step_applied={update_successful}")
            except Exception as e:
                print_rank_0(f"[ADAPTDL_GNS_DEBUG][rank0] ds debug failed: {e}")

        iteration_num = getattr(args, 'curr_iteration', getattr(args, 'iteration', 0))
        if iteration_num <= 10:
            print_rank_0(f"[ADAPTDL_GNS_DEBUG] iter={iteration_num} _ADAPTDL_GNS={_ADAPTDL_GNS is not None} is_rank_0={is_rank_0()} is_last_rank={is_last_rank()} update_successful={update_successful}")
        if _ADAPTDL_GNS is not None and (is_rank_0() or is_last_rank()):
            if update_successful:
                init_bsz = _ADAPTDL_INIT_BATCH_SIZE or args.global_batch_size
                try:
                    scale = args.global_batch_size / float(init_bsz)
                    gain = float(_ADAPTDL_GNS.gain(scale))
                    efficiency = gain / float(scale)

                    grad_sqr = float(_ADAPTDL_GNS.sqr_avg())
                    grad_var = float(_ADAPTDL_GNS.var_avg())
                    _cona_record_gns(grad_sqr, grad_var)

                    if iteration_num <= 50:
                        print_rank_0(f"[ADAPTDL_GNS_DEBUG] iter={iteration_num} grad_sqr={grad_sqr:.6e} grad_var={grad_var:.6e} scale={scale:.3f} update_successful={update_successful}")

                    efficiency_dict = {}

                    if grad_sqr > 0 and grad_var >= 0 and not (math.isinf(grad_sqr) or math.isnan(grad_sqr) or math.isinf(grad_var) or math.isnan(grad_var)):
                        try:
                            ...
                            goodput_fn = GoodputFunction(dummy_perf_params, grad_params, init_bsz)

                            for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES:
                                if bs >= init_bsz:  # Efficiency only defined for batch_size >= init_batch_size
                                    try:
                                        bs_efficiency = goodput_fn.efficiency(bs)
                                        if isinstance(bs_efficiency, np.ndarray):
                                            bs_efficiency = float(bs_efficiency.item())
                                        else:
                                            bs_efficiency = float(bs_efficiency)
                                        if not (math.isinf(bs_efficiency) or math.isnan(bs_efficiency) or bs_efficiency < 0):
                                            efficiency_dict[bs] = bs_efficiency
                                    except Exception:
                                        pass
                        except ImportError:
                            import numpy as np
                            # Fallback mirrors GoodputFunction.efficiency() from
                            # AdaptDL (adaptdl/goodput.py), Copyright 2020 Petuum, Inc.,
                            # Apache License 2.0 (see utils/adaptdl/LICENSE).
                            for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES:
                                if bs >= init_bsz:
                                    scale = float(bs) / float(init_bsz)
                                    if scale > 0:
                                        denom = grad_var / scale + grad_sqr
                                        gain = np.where(denom > 0, (grad_var + grad_sqr) / denom, 1.0)
                                        bs_efficiency = gain / scale
                                        if isinstance(bs_efficiency, np.ndarray):
                                            bs_efficiency = float(bs_efficiency.item())
                                        else:
                                            bs_efficiency = float(bs_efficiency)
                            ...
                            if iteration_num <= 10:
                                print_rank_0(f"[ADAPTDL_GNS] efficiency calculation failed: {eff_err}")
                    else:
                        efficiency_dict = {}
                    ...
                    if is_last_rank():
                        try:
                            ...
                            if wandb_writer is not None:
                                init_bsz_wb = _ADAPTDL_INIT_BATCH_SIZE or args.global_batch_size
                                scale_wb = args.global_batch_size / float(init_bsz_wb)
                                gain_wb = float(_ADAPTDL_GNS.gain(scale_wb))
                                efficiency_wb = gain_wb / float(scale_wb)
                                grad_sqr_wb = float(_ADAPTDL_GNS.sqr_avg())
                                grad_var_wb = float(_ADAPTDL_GNS.var_avg())

                                efficiency_dict_wb = {}
                                if grad_sqr_wb > 0 and grad_var_wb >= 0 and not (math.isinf(grad_sqr_wb) or math.isnan(grad_sqr_wb) or math.isinf(grad_var_wb) or math.isnan(grad_var_wb)):
                                    try:
                                        from adaptdl.adaptdl.goodput import GoodputFunction
                                        import numpy as np
                                        dummy_perf_params = (0.1, 0.01, 0.1, 0.01, 0.1, 0.01, 1.0)
                                        grad_params = (grad_sqr_wb, grad_var_wb)
                                        goodput_fn = GoodputFunction(dummy_perf_params, grad_params, init_bsz_wb)
                                        for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES:
                                            if bs >= init_bsz_wb:
                                                try:
                                                    bs_efficiency = goodput_fn.efficiency(bs)
                                                    if isinstance(bs_efficiency, np.ndarray):
                                                        ...
                                                        bs_efficiency = float(bs_efficiency)
                                                    if not (math.isinf(bs_efficiency) or math.isnan(bs_efficiency) or bs_efficiency < 0):
                                                        efficiency_dict_wb[bs] = bs_efficiency
                                                except Exception:
                                                    pass
                                    except ImportError:
                                        import numpy as np
                                        for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES:
                                            if bs >= init_bsz_wb:
                                                scale_bs = float(bs) / float(init_bsz_wb)
                                                if scale_bs > 0:
                                                    denom = grad_var_wb / scale_bs + grad_sqr_wb
                                                    gain_bs = np.where(denom > 0, (grad_var_wb + grad_sqr_wb) / denom, 1.0)
                                                    bs_efficiency = gain_bs / scale_bs
                                                    if isinstance(bs_efficiency, np.ndarray):
                                                        bs_efficiency = float(bs_efficiency.item())
                                                    else:
                                                        bs_efficiency = float(bs_efficiency)
                                ...
                                try:
                                    wandb_writer.log(
                                        {
                                            "adaptdl/grad_sqr": grad_sqr_wb,
                                            "adaptdl/grad_var": grad_var_wb,
                                            "adaptdl/scale_current_gbs": float(scale_wb),
                                            "adaptdl/gain_current_gbs": float(gain_wb),
                                            "adaptdl/efficiency_current_gbs": float(efficiency_wb),
                                        },
                                        step=iteration_num,
                                    )
                                    if iteration_num <= 10:
                                        print_rank_0(f"[ADAPTDL_GNS] wandb logged: grad_sqr={grad_sqr_wb:.6e} grad_var={grad_var_wb:.6e} scale={scale_wb:.3f} gain={gain_wb:.6e} efficiency={efficiency_wb:.6e}")

                                    if efficiency_dict_wb:
                                        per_bs = {f"adaptdl/efficiency_bs{bs}": float(val)
                                                  for bs, val in efficiency_dict_wb.items()}
                                        wandb_writer.log(per_bs, step=iteration_num)
                                        if iteration_num <= 10:
                                            print_rank_0(f"[ADAPTDL_GNS] wandb logged per-bs efficiencies: {list(per_bs.keys())}")
                                except Exception as wandb_err:
                                    if iteration_num <= 10:
                                        print_rank_0(f"[ADAPTDL_GNS] wandb log failed: {wandb_err}")
                                        import traceback
                                        print_rank_0(f"[ADAPTDL_GNS] wandb traceback: {traceback.format_exc()}")
                        except Exception as e:
                            if iteration_num <= 10:
                                print_rank_0(f"[ADAPTDL_GNS] get_wandb_writer() failed: {e}")
                except Exception as e:
                    print_rank_0(f"[ADAPTDL_GNS] log failed: {e}")
                    import traceback
                    print_rank_0(f"[ADAPTDL_GNS] traceback: {traceback.format_exc()}")
            else:
                efficiency_dict = {}  # Initialize empty dict
                if _ADAPTDL_EFFICIENCY_CSV_FILE and os.path.exists(_ADAPTDL_EFFICIENCY_CSV_FILE):
                    try:
                        import csv
                        with open(_ADAPTDL_EFFICIENCY_CSV_FILE, 'r') as csvfile:
                            reader = csv.reader(csvfile)
                            rows = list(reader)
                            if len(rows) > 1:  # Has header + at least one data row
                                last_row = rows[-1]
                                if len(last_row) > 1 and last_row[0].isdigit():
                                    for i, bs in enumerate(_ADAPTDL_EFFICIENCY_BATCH_SIZES, start=1):
                                        if i < len(last_row) and last_row[i]:
                                            try:
                                                efficiency_dict[bs] = float(last_row[i])
                                            except (ValueError, IndexError):
                                                pass
                    except Exception:
                        pass
                if iteration_num <= 10:
                    print_rank_0(f"[ADAPTDL_GNS_DEBUG] iter={iteration_num} update_successful=False, using prev values: {len(efficiency_dict)} values")

            if _ADAPTDL_EFFICIENCY_CSV_FILE:
                        try:
                            import csv
                            file_exists = os.path.exists(_ADAPTDL_EFFICIENCY_CSV_FILE)
                            with open(_ADAPTDL_EFFICIENCY_CSV_FILE, 'a', newline='') as csvfile:
                                writer = csv.writer(csvfile)

                                if not _ADAPTDL_CSV_HEADER_WRITTEN or not file_exists:
                                    header = ['iteration #', 'grad_sqr', 'grad_var'] + [f'efficiency(bs={bs})' for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES]
                                    writer.writerow(header)
                                    _ADAPTDL_CSV_HEADER_WRITTEN = True

                                try:
                                    _csv_gs = float(_ADAPTDL_GNS.sqr_avg())
                                    _csv_gv = float(_ADAPTDL_GNS.var_avg())
                                except Exception:
                                    _csv_gs, _csv_gv = '', ''

                                row = [iteration_num, _csv_gs, _csv_gv] + [efficiency_dict.get(bs, '') for bs in _ADAPTDL_EFFICIENCY_BATCH_SIZES]
                                writer.writerow(row)

                                if iteration_num <= 50:
                                    print_rank_0(f"[ADAPTDL_GNS_DEBUG] iter={iteration_num} CSV written: {len(efficiency_dict)} efficiencies calculated")
                        except Exception as csv_err:
                            print_rank_0(f"[ADAPTDL_GNS] CSV write failed: {csv_err}")
                            import traceback
                            print_rank_0(f"[ADAPTDL_GNS] CSV traceback: {traceback.format_exc()}")


        if _ADAPTDL_GNS is not None:
            try:
                _ADAPTDL_GNS.reset_accumulation()
            except Exception:
                pass
    ...
    if args.deepspeed:
        ...
        iteration_num = getattr(args, 'curr_iteration', getattr(args, 'iteration', 0))
        if iteration_num <= 10:
            print_rank_0(f"[ADAPTDL_GNS_DEBUG] iter={iteration_num} _ADAPTDL_GNS={_ADAPTDL_GNS is not None} is_rank_0={is_rank_0()} update_successful={update_successful}")
        if _ADAPTDL_GNS is not None and is_rank_0():
            if update_successful:
                init_bsz = _ADAPTDL_INIT_BATCH_SIZE or args.global_batch_size
                try:
                    scale = args.global_batch_size / float(init_bsz)
        ...
        if _ADAPTDL_GNS is not None:
            if args.rank == (args.world_size - 1):
                try:
                    wandb_writer = get_wandb_writer()
                    if iteration_num <= 10:
                        print(f"[ADAPTDL_GNS_DEBUG] rank={args.rank} iter={iteration_num} get_wandb_writer() returned: {wandb_writer is not None}", flush=True)
                except Exception as e:
                    wandb_writer = None
                    if iteration_num <= 10:
                        print(f"[ADAPTDL_GNS] rank={args.rank} iter={iteration_num} get_wandb_writer() failed: {e}", flush=True)
            else:
                wandb_writer = None
                ...
                try:
                    ...
                    if grad_sqr_wb > 0 and grad_var_wb >= 0 and not (math.isinf(grad_sqr_wb) or math.isnan(grad_sqr_wb) or math.isinf(grad_var_wb) or math.isnan(grad_var_wb)):
                        try:
                            ...
                            if iteration_num <= 10:
                                print_rank_0(f"[ADAPTDL_GNS] W&B efficiency calculation failed: {eff_err}")
                    ...
                    # Always log success message (not just first 10 iterations)
                    print_rank_0(f"[ADAPTDL_GNS] wandb logged: grad_sqr={grad_sqr_wb:.6e} grad_var={grad_var_wb:.6e} scale={scale_wb:.3f} gain={gain_wb:.6e} efficiency={efficiency_wb:.6e}")
                    ...
                    if iteration_num <= 10:
                        ...
                        import traceback
                        print_rank_0(f"[ADAPTDL_GNS] wandb traceback: {traceback.format_exc()}")
            elif wandb_writer is None:
                if iteration_num <= 10:
                    print_rank_0(f"[ADAPTDL_GNS] wandb_writer is None (W&B initialized on last rank only), skipping W&B log")

        # Per-step accumulation is reset after every step, applied or skipped,
        # so that GradientNoiseScale does not carry gradients across steps.
        if _ADAPTDL_GNS is not None:
            try:
                _ADAPTDL_GNS.reset_accumulation()
            except Exception:
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if writer.is_enabled() and (iteration % args.tensorboard_log_interval == 0):
        ...
        if args.log_batch_size_to_tensorboard:
            ...
            writer.add_scalar('batch-size/batch-size vs tokens', batch_size,
                              args.consumed_train_tokens, x_axis_tokens)
        for key in loss_dict:
            # User preference: SOP/NSP-style loss isn't useful to track.
            if 'sop' in str(key).lower():
                continue
    ...
    if iteration % args.log_interval == 0:
        ...
        if writer.is_enabled():
            ...
            # Averaging the logged loss is opt-in through MEGATRON_WANDB_AVG_LOSS;
            # otherwise the raw loss tensors are logged.
            import os as _os
            avg_loss_to_wandb = (_os.environ.get("MEGATRON_WANDB_AVG_LOSS", "0") == "1")

            if loss_dict is not None:
                if avg_loss_to_wandb:
                    # Log the *same averaged losses* that stdout prints.
                    advanced = float(max(1, total_loss_dict.get(advanced_iters_key, 0)))
                    for k, v in loss_dict.items():
                        # User preference: SOP/NSP-style loss isn't useful to track on W&B.
                        if 'sop' in str(k).lower():
                            continue

                        # Raw instantaneous loss (debug).
                        writer.add_scalar_to_wandb(f'loss_raw/{k}', v, iteration)

                        # Smoothed/averaged loss over the log interval (matches stdout).
                        if k in total_loss_dict:
                            try:
                                avg_v = float(total_loss_dict[k].item()) / advanced
                            except Exception:
                                avg_v = float(v)
                        else:
                            avg_v = float(v)

                        writer.add_scalar_to_wandb(f'loss/{k}', avg_v, iteration)

                        # MLM loss and accuracy (for BERT): log separately for clarity.
                        if str(k).lower() == 'lm loss':
                            # MLM loss (same as lm loss for BERT)
                            writer.add_scalar_to_wandb(f'mlm-loss/{k}', avg_v, iteration)
                            writer.add_scalar_to_wandb(f'mlm-loss/{k} vs samples', avg_v, args.consumed_train_samples, x_axis_samples)
                            writer.add_scalar_to_wandb(f'mlm-loss/{k} vs tokens', avg_v, args.consumed_train_tokens, x_axis_tokens)
                            # MLM PPL proxy (for BERT) based on averaged lm loss.
                            try:
                                writer.add_scalar_to_wandb('ppl/train_lm', math.exp(min(20.0, float(avg_v))), iteration)
                            except Exception:
                                pass
                        # MLM accuracy (for BERT): log separately as accuracy metric.
                        if 'mlm acc' in str(k).lower():
                            writer.add_scalar_to_wandb(f'mlm-accuracy/{k}', avg_v, iteration)
                            writer.add_scalar_to_wandb(f'mlm-accuracy/{k} vs samples', avg_v, args.consumed_train_samples, x_axis_samples)
                            writer.add_scalar_to_wandb(f'mlm-accuracy/{k} vs tokens', avg_v, args.consumed_train_tokens, x_axis_tokens)
                else:
                    # Historical behavior (do NOT change GPT runs): log raw loss values.
                    for k, v in loss_dict.items():
                        if 'sop' in str(k).lower():
                            continue
                        ...
                        if str(k).lower() == 'lm loss':
                            # MLM loss (same as lm loss for BERT)
                            writer.add_scalar_to_wandb(f'mlm-loss/{k}', v, iteration)
                        # MLM accuracy (for BERT): log separately as accuracy metric.
                        if 'mlm acc' in str(k).lower():
                            writer.add_scalar_to_wandb(f'mlm-accuracy/{k}', v, iteration)
        ...
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key,
                           nan_iters_key]:
                # User preference: drop SOP/NSP-style loss from stdout logs.
                if 'sop' in str(key).lower():
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters and (args.train_tokens is None or \
        ...
        # this job is running, so it checkpoints at this iteration boundary and
        cona_action = _cona_maybe_switch(iteration)
        if cona_action:
            if args.save and not saved_checkpoint:
                save_checkpoint_and_time(iteration, model, optimizer,
                                         opt_param_scheduler)
            torch.distributed.barrier()
            _cona_record_switch(iteration, cona_action)
            print_datetime(f"CONA {cona_action} at iteration={iteration}. "
                           "Exiting")
            megatron_quiesce_before_exit()
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    for key in total_loss_dict:
        ...
        # Only compute a PPL-style metric for actual losses.
        # (e.g., for accuracy metrics like 'mlm acc', exp() is meaningless.)
        key_l = str(key).lower()
        ppl = None
        if ('loss' in key_l) and ('acc' not in key_l):
            ppl = math.exp(min(20, total_loss_dict[key].item()))
            string += '{} PPL: {:.6E} | '.format(key, ppl)
        if writer.is_enabled() and is_last_rank():
            # User preference: keep SOP in stdout but do not log it to W&B/TB.
            if 'sop' in str(key).lower():
                continue
            ...
            writer.add_scalar(f'lm-loss-validation/{key} {data_type}',
                              ...
                              iteration)
            # Log PPL to W&B explicitly (only for true loss keys).
            if ppl is not None:
                try:
                    writer.add_scalar_to_wandb(f'ppl/{data_type}_{key}', ppl, iteration)
                except Exception:
                    pass
            ...
            writer.add_scalar(f'lm-loss-validation/{key} {data_type} vs tokens',
                              ...
                              x_axis_tokens)
            # MLM loss and accuracy (for BERT): log separately for clarity.
            if str(key).lower() == 'lm loss':
                # MLM loss (same as lm loss for BERT)
                writer.add_scalar_to_wandb(f'mlm-loss-validation/{key} {data_type}',
                                          total_loss_dict[key].item(),
                                          iteration)
                writer.add_scalar_to_wandb(f'mlm-loss-validation/{key} {data_type} vs samples',
                                          total_loss_dict[key].item(),
                                          args.consumed_train_samples,
                                          x_axis_samples)
                writer.add_scalar_to_wandb(f'mlm-loss-validation/{key} {data_type} vs tokens',
                                          total_loss_dict[key].item(),
                                          args.consumed_train_tokens,
                                          x_axis_tokens)
            # MLM accuracy (for BERT): log separately as accuracy metric.
            if 'mlm acc' in str(key).lower():
                writer.add_scalar_to_wandb(f'mlm-accuracy-validation/{key} {data_type}',
                                          total_loss_dict[key].item(),
                                          iteration)
                writer.add_scalar_to_wandb(f'mlm-accuracy-validation/{key} {data_type} vs samples',
                                          total_loss_dict[key].item(),
                                          args.consumed_train_samples,
                                          x_axis_samples)
                writer.add_scalar_to_wandb(f'mlm-accuracy-validation/{key} {data_type} vs tokens',
                                          total_loss_dict[key].item(),
                                          args.consumed_train_tokens,
                                          x_axis_tokens)
