    ...
    def _build_universal_checkpoint_info(self):
        ...
        if DS_UNIVERSAL_CHECKPOINT_INFO:
            ...
            if self.args.tensor_model_parallel_size > 1:
                # IMPORTANT:
                # DeepSpeed's ds_to_universal converter asserts exact equality (torch.equal) across TP slices
                # for tp_replicated patterns. In practice, some TP-replicated parameters may not be bitwise
                # identical, which breaks TP>1 -> universal conversion.
                #
                # To make conversion robust *without modifying DeepSpeed*, classify these patterns as
                # parameter_to_average_patterns so ds_to_universal averages TP slices instead.
                info[PARAMETER_TO_AVERAGE_PATTERNS] = self._get_tp_replicated_param_patterns()
