...
class OptimizerParamScheduler(object):
    ...
    def __init__(self, optimizer, max_lr, min_lr,
        ...
        self.min_lr = min_lr
        # Keep a copy of input lr values so we can optionally override only lr
        # after loading scheduler state from a checkpoint (BERT-only staged runs).
        self._input_max_lr = float(max_lr)
        self._input_min_lr = float(min_lr)
        ...
        # BERT-only: allow staged runs to keep scheduler state from checkpoint
        # but override only lr/min-lr from input args after loading.
        self.bert_override_lr_only = bool(getattr(args, 'bert_override_lr_only', False))
    ...
    def get_lr(self):
        ...
        if self.lr_decay_tokens is None:
            ...
            warmup_tokens_ = self.lr_warmup_tokens or 0
            num_tokens_ = self.num_tokens - warmup_tokens_
            decay_tokens_ = self.lr_decay_tokens - warmup_tokens_
    ...
    def load_state_dict(self, sd):
        ...
        # BERT-only staged runs:
        # If user requested it, keep everything from checkpoint (including
        # warmup/decay schedule and progress) but override max_lr/min_lr from
        # the current run's args, then re-apply lr at the same token position.
        #
        # This enables changing LR between stages (e.g., sqrt scaling by GBS)
        # without breaking checkpoint loading, and has no effect unless the
        # flag is explicitly set.
        if self.bert_override_lr_only:
            try:
                old_max_lr, old_min_lr = self.max_lr, self.min_lr
                self.max_lr = self._input_max_lr
                self.min_lr = self._input_min_lr
                print_rank_0(
                    f' > [BERT] overriding lr/min-lr from checkpoint '
                    f'({old_max_lr}, {old_min_lr}) -> input args '
                    f'({self.max_lr}, {self.min_lr})'
                )
                # Re-apply lr at current progress (do not change num_steps).
                self.step(increment=0, token_num=self.num_tokens)
            except Exception:
                # Never fail checkpoint loading due to this optional feature.
                pass
