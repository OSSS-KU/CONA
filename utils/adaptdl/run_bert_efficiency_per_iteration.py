#!/usr/bin/env python3
"""
BERT Efficiency Calculator - Per Iteration
Runs BERT training and calculates efficiency for all batch sizes at each iteration.
...
def run_bert_training(batch_size, micro_batch_size, megatron_dir, output_dir,
                     train_iters=5000, seq_length=256, init_batch_size=1,
                     target_batch_sizes=[8,16,32,64,128,256],
                     dp_launch=4, tp=1, pp=1):
    """Run BERT training with specified batch size."""
    print(f"\n{'='*80}")
    print(f"Running BERT training: batch_size={batch_size}, micro_batch_size={micro_batch_size}")
    print(f"{'='*80}")
    
    output_dir = os.path.abspath(output_dir)
    os.makedirs(output_dir, exist_ok=True)
    
    log_file = os.path.join(output_dir, f"bert_bs{batch_size}.log")
    ...
    # BERT data paths (updated to cona layout)
    data_path = "/workspace/cona/Megatron-DeepSpeed/univ_ckpt/bert/data/wikitext_bert_large_docsent_v10_text_document"
    vocab_file = "/workspace/cona/Megatron-DeepSpeed/univ_ckpt/bert/bert_vocab_wikitext_30000.txt"
    ...
    cmd = [
        ...
        "--lr", "5e-5",  # Reduced LR to prevent loss spikes and GNS resets (was 7.5e-5)
        "--min-lr", "1e-5",
        "--lr-decay-style", "cosine",
        "--lr-warmup-fraction", "0.1",  # Longer warmup (10% of training) for smoother initial phase
        "--lr-decay-iters", str(train_iters),  # Use iteration-based decay for consistency
        "--weight-decay", "0.01",
        "--mask-prob", "0.15",
        "--short-seq-prob", "0.10",
    ...
    try:
        ...
        with open(log_file_abs, 'w') as f:
            ...
            env["NO_CKPT"] = "1"
            # Skip GNS reset by default, matching the GPT behavior.
            env["ADAPTDL_GNS_SKIP_RESET"] = "0"
            
            # Enable AdaptDL GNS logging and CSV generation (like GPT benchmark)
...
def main():
    parser = argparse.ArgumentParser(description='BERT Efficiency Calculator (Per Iteration)')
    ...
    parser.add_argument('--train-iters', type=int, default=5000,
    ...
    parser.add_argument('--init-batch-size', type=int, default=1,
                       help='Baseline batch size for efficiency calculation (default: 1)')
    ...
    if args.output_dir:
        ...
        run_dir_name = f"bert_dp{dp}_tp{tp}_pp{pp}_gbs{run_batch_size}_mbs{micro_batch_size}"
        run_dir = base_results / run_dir_name
    run_dir.mkdir(parents=True, exist_ok=True)
    
    print("="*80)
    print("BERT Efficiency Calculator (Per Iteration, Pollux/AdaptDL Method)")
    print("="*80)
